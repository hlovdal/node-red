...
RED.editor.codeEditor.monaco = (function() {
    ...
    function _loadModuleDTS(mod, preloadOnly, loadedLibs, cb) {
        ...
        if(_module) {
            ...
            if( def ) {
                if(!preloadOnly) {
                    loadedLibs.JS[libModule] = monaco.typescript.javascriptDefaults.addExtraLib(def, "file://types/" + libPackage + "/" + libModule + "/index.d.ts");
                }
                if(cb) {
                    setTimeout(function() {
                        cb(null, _module);
    ...
    function init(options) {
        ...
        function setupJSON(_monaco) {
            ...
            try {
                ...
                /** @see https://microsoft.github.io/monaco-editor/docs.html#interfaces/languages_features_json_register.DiagnosticsOptions.html */ 
                var diagnosticOptions = RED.settings.get('codeEditor.monaco.json.jsonDefaults.diagnosticOptions');
                /** @see https://microsoft.github.io/monaco-editor/docs.html#interfaces/languages_features_json_register.ModeConfiguration.html */
                var modeConfiguration = RED.settings.get('codeEditor.monaco.json.jsonDefaults.modeConfiguration');
                diagnosticOptions = Object.assign({}, diagnosticOptionsDefault, (diagnosticOptions || {}));
                _monaco.json.jsonDefaults.setDiagnosticsOptions(diagnosticOptions);
                if(modeConfiguration) { _monaco.json.jsonDefaults.setModeConfiguration(modeConfiguration); }
            } catch (error) {
        ...
        function setupHTML(_monaco) {
            ...
            try {
                var htmlDefaults = RED.settings.get('codeEditor.monaco.html.htmlDefaults.options');
                var handlebarDefaults = RED.settings.get('codeEditor.monaco.html.handlebarDefaults.options');
                var htmlModeConfiguration = RED.settings.get('codeEditor.monaco.html.htmlDefaults.modeConfiguration');
                var handlebarModeConfiguration = RED.settings.get('codeEditor.monaco.html.handlebarDefaults.modeConfiguration');
                if(htmlDefaults) { _monaco.html.htmlDefaults.setOptions(htmlDefaults); }
                if(handlebarDefaults) { _monaco.html.handlebarDefaults.setOptions(handlebarDefaults); }
                if(htmlModeConfiguration) { _monaco.html.htmlDefaults.setModeConfiguration(htmlModeConfiguration); }
                if(handlebarModeConfiguration) { _monaco.html.handlebarDefaults.setModeConfiguration(handlebarModeConfiguration); }
            } catch (error) {
        ...
        function setupCSS(_monaco) {
            ...
            try {
                var cssDefaults_option = RED.settings.get('codeEditor.monaco.css.cssDefaults.options');
                var lessDefaults_option = RED.settings.get('codeEditor.monaco.css.lessDefaults.options');
                var scssDefaults_option = RED.settings.get('codeEditor.monaco.css.scssDefaults.options');
                var cssDefaults_modeConfiguration = RED.settings.get('codeEditor.monaco.css.cssDefaults.modeConfiguration');
                var lessDefaults_modeConfiguration = RED.settings.get('codeEditor.monaco.css.lessDefaults.modeConfiguration');
                var scssDefaults_modeConfiguration = RED.settings.get('codeEditor.monaco.css.scssDefaults.modeConfiguration');
                if(cssDefaults_option) { _monaco.css.cssDefaults.setOptions(cssDefaults_option); }
                if(lessDefaults_option) { _monaco.css.lessDefaults.setOptions(lessDefaults_option); }
                if(scssDefaults_option) { _monaco.css.scssDefaults.setOptions(scssDefaults_option); }
                if(cssDefaults_modeConfiguration) { _monaco.css.cssDefaults.setModeConfiguration(cssDefaults_modeConfiguration); }
                if(lessDefaults_modeConfiguration) { _monaco.css.lessDefaults.setModeConfiguration(lessDefaults_modeConfiguration); }
                if(scssDefaults_modeConfiguration) { _monaco.css.scssDefaults.setModeConfiguration(scssDefaults_modeConfiguration); }
            } catch (error) {
        ...
        function setupJS(_monaco) {
            ...
            try {
                ...
                //apply overrides from codeEditor.monaco.typescript.javascriptDefaults.compilerOptions in settings.js
                var settingsComilerOptions = RED.settings.get('codeEditor.monaco.typescript.javascriptDefaults.compilerOptions') || {};
                compilerOptions = Object.assign({}, compilerOptions, settingsComilerOptions);
                /** @see https://microsoft.github.io/monaco-editor/docs.html#interfaces/languages_features_typescript_register.LanguageServiceDefaults.html#setCompilerOptions */
                _monaco.typescript.javascriptDefaults.setCompilerOptions(compilerOptions);
                ...
                //apply overrides from codeEditor.monaco.typescript.javascriptDefaults.diagnosticsOptions settings.js
                var settingsDiagnosticsOptions = RED.settings.get('codeEditor.monaco.typescript.javascriptDefaults.diagnosticsOptions') || {};
                diagnosticOptions = Object.assign({}, diagnosticOptions, settingsDiagnosticsOptions);
                /** @see https://microsoft.github.io/monaco-editor/docs.html#interfaces/languages_features_typescript_register.LanguageServiceDefaults.html#setDiagnosticsOptions */
                _monaco.typescript.javascriptDefaults.setDiagnosticsOptions(diagnosticOptions);
            } catch (error) {
        ...
        initialised = true;
        // Apply dark theme if the dark theme is already active (e.g. restored from localStorage on page load)
        if (!userSelectedTheme && document.documentElement.classList.contains('nr-theme-dark')) {
    ...
    function create(options) {
        ...
        if (editorOptions.foldStyle == null) { editorOptions.foldStyle = true; }
        ...
        if (editorOptions.theme == null) {
            editorOptions.theme = document.documentElement.classList.contains('nr-theme-dark') ? 'vs-dark' : monacoThemes[0];
        }
        ...
        editorOptions.snippetSuggestions = editorOptions.enableSnippets === false ? false : true; //default to true
