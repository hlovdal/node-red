...
RED.popover = (function() {
    ...
    return {
        ...
        tooltip: function(target,content, action, interactive) {
            // Hover tooltips aren't an accessible name, so mirror the text to aria-label.
            var applyAriaLabel = function(text) {
                if (typeof text !== 'string' || !text || !target || !target.attr) { return; }
                var labelTarget = target;
                if (!target.is('button, a, input, [role=button], [role=link]')) {
                    var inner = target.find('button, a, [role=button], [role=link]').first();
                    if (inner.length) { labelTarget = inner; }
                }
                if (labelTarget.attr('aria-labelledby')) { return; }
                var existing = labelTarget.attr('aria-label');
                var managed = labelTarget.data('red-ui-tooltip-managed');
                if (existing && !managed) { return; }
                labelTarget.attr('aria-label', text);
                labelTarget.data('red-ui-tooltip-managed', true);
            }
            var label = function() {
                var label = content;
                if (typeof content === 'function') {
            ...
            popover.setContent = function(newContent) {
                content = newContent;
                applyAriaLabel(typeof newContent === 'function' ? newContent() : newContent);
            }
        ...
        dialog: function(options) {
            ...
            if (options.closeButton !== false) {
                $('<button type="button" class="red-ui-button red-ui-button-small" style="float: right; margin-top: -4px; margin-right: -4px;"><i class="fa fa-times"></i></button>').attr('aria-label', RED._('common.label.close')).appendTo(dialogContent).click(function(evt) {
                    evt.preventDefault();
                    close();
                })
            }
